...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QToolBar,
                             ...
                             QPushButton, QHBoxLayout, QMessageBox, QFileDialog, QAction)
...
class VentanaLogin(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Chronioñ ID & Accounts (x86)")
...
class ChronionCompleto(QMainWindow):
    def __init__(self, usuario, email):
        super().__init__()
        self.usuario_sesion = usuario
        self.email_sesion = email
        self.setWindowTitle(f"Chronioñ Browser v1.2 (x86) - [{self.usuario_sesion}]")
...
if __name__ == "__main__":
    ...
    if login.exec_() == QDialog.Accepted:
        mi_navegador = ChronionCompleto(login.usuario_actual, login.email_actual)
        mi_navegador.show()
        sys.exit(app.exec_())
